...
def interactive_setup():
    ...
    # 2. 码表路径
    print(f"\n2. 码表文件路径")
    print("   码表包含：城市、品牌、运营主体、对接人 的映射关系")
...
def load_mabiao(mabiao_path, target_duijieren):
    ...
    df = pd.read_excel(mabiao_path)

    # 解析对接人
    if target_duijieren == "全部":
        df_filtered = df
        duijieren_list = df["对接人"].dropna().unique().tolist()
        print(f"拆出所有对接人: {duijieren_list}")
    else:
        # 支持逗号分隔的多个对接人
        duijieren_list = [x.strip() for x in target_duijieren.split(",")]
        df_filtered = df[df["对接人"].isin(duijieren_list)]
    ...
    for _, row in df_filtered.iterrows():
        key = (row["品牌"], row["城市"])
        mapping[key] = row["新-运营主体"]

    # 获取运营主体列表
    zhuti_list = df_filtered["新-运营主体"].unique().tolist()
...
def save_split_files(zhuti_data, original_name, output_dir):
...
def main():
    ...
    # 加载码表
    try:
        mapping, zhuti_list = load_mabiao(mabiao_path, duijieren)
        if not mapping:
    ...
    for file_path in todo_files:
        ...
        zhuti_data = process_excel(file_path, mapping, zhuti_list, file_config)
        saved = save_split_files(zhuti_data, file_path.name, temp_dir)
    ...
    zip_name = f"拆分结果_{timestamp}.zip"
